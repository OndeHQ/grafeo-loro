...
// Issue #4: `LineageEpoch`, `EpochMismatchError`, `OfflineOpQueue`, and the
// new `EpochTracker` now live in `crate::bridge::queue` (gated only by
// `feature = "bridge"` — no tokio/grafeo/telemetry). They are `pub use`-d
// here so existing call sites that import from `bridge::sync_engine::*`
// (e.g. `tests/graph_sync.rs`) keep compiling without modification. The
// `SyncEngine` itself holds an `Arc<EpochTracker>` + `Arc<Mutex<OfflineOpQueue>>`
// and delegates its queue/epoch methods to those fields.
pub use crate::bridge::queue::{EpochMismatchError, EpochTracker, LineageEpoch, OfflineOpQueue};
...
pub struct SyncEngine {
    ...
    /// Lineage epoch tracker (issue #4 — factored out into `crate::bridge::queue`).
    /// Bumped on every "cache wipe" event — `wipe_cache()`, server reset
    /// signal from orchestrator, or any lineage break detected at sync
    /// handshake. Exchanged at sync handshake: if `local != remote`, the
    /// client MUST wipe its cache before syncing (see
    /// [`Self::check_epoch_match`]). Held as `Arc<EpochTracker>` so the
    /// WASM-only `EpochTracker` type can be shared with non-`SyncEngine`
    /// callers (orchestrator FFI).
    pub(crate) epoch_tracker: Arc<EpochTracker>,
    /// Native offline op-queue (issue #3 sub-issue 5 / issue #4 — factored
    /// out into `crate::bridge::queue`). Replaces downstream's custom IDB
    /// queue + exponential backoff. 10 MB cap. Exposed via
...
impl SyncEngine {
    ...
    fn new_inner(
        ...
        let engine = Self {
            ...
            tracer,
            health,
            epoch_tracker: Arc::new(EpochTracker::new()),
    ...
    pub fn check_epoch_match(
        ...
        self.epoch_tracker.check_match(remote_epoch)
    }
    ...
    pub fn wipe_cache(&self) -> LineageEpoch {
        let new_epoch = self.epoch_tracker.wipe();
