...
/// fuzz iteration speed). The 5 variants map 1:1 to `LoroProperty` variants
/// via `From<FuzzValue> for GraphValue`.
#[derive(Arbitrary, Debug, Clone)]
pub enum FuzzValue {
    Null,
    Bool(bool),
...
impl FuzzValue {
    ...
    fn to_graph_value(&self) -> GraphValue {
        match self {
...
fn convert_fuzz_op(op: &FuzzOp) -> LoroOp {
    match op {
        FuzzOp::UpsertNode {
            loro_key,
            labels,
